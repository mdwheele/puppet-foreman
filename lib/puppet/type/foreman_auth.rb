...
Puppet::Type.newtype(:foreman_auth) do
  desc 'foreman_auth manages LDAP authentication sources in foreman.'
  ensurable
  ...
  newproperty(:groups_base) do
    desc 'Top level DN of your LDAP directory tree for group lookups'
  end
  ...
  newproperty(:usergroup_sync, :parent => Puppet::Property::Boolean) do
    desc 'External user groups will be synced on login.'
  end
